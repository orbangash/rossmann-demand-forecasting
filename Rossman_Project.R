...
# --- 5.4 Decision Tree ---
# Fit the model tree (depth 10) - used for prediction and RMSE
tree_model <- rpart(Sales ~ .,
                    data    = train_set,
                    method  = "anova",
                    control = rpart.control(maxdepth = 10))

# A depth-10 tree with 1,115 store labels is unreadable on a page,
# so fit a separate shallow depth-3 tree purely for visualisation.
...
rpart.plot(tree_simple,
           main  = "Decision Tree Structure (Illustrative Depth-3 Tree)",
           type  = 4,
           extra = 101,
           tweak = 1.1)
